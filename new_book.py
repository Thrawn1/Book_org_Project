...
#Анлогично и для других атрибутов. Функции защиты от некорректных данных будут дорабатываться. 
#Далее идет проверка переменной p. Эта переменная по умаолчанию равна 0, и несет в себе смысл, что мы заполняем поля для однотомной книги. В этом случае после заврешения цикла заполнения полей атрибутов происходит возврат в предыдущую функцию экземпляра класса с уже заполненными правильно атрибутами. Если p равен 1(то есть мы вызываем функцию и передаем ей в качестве аргумнета 1, то запускается цикл заполнения специфичных для многотомной книги атрибутов. 
