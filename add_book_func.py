...
def add_book_func(db_f):
        ...
        print('Всего записей в базе данных:',d)
...
#В зависимости от того, какой пункт выбран, переменной j присваевается пустой экземпляр соответсвующего класса. Далее эта переменная передается в функцию new_book, из котрой возвращается экземпляр класса с заполненными атрибутами(пременная t). В случае с многотомной книгой в функцию new_book передается 1, котрая запускает условие, по котрому заполняются дополнительные атрибуты присущие только многотомной книге. Это было сделано для того, что бы использовать всего одну функцию заполнение атрибутов экземпляра класса. 
#Далее экземпляр класса в переменной t записывается в базу данных по ключу y. 
#Затем происходит вывод информационого сообщения что запись занесена, и вывод последних 15 записей.
#В переменную M возвращается сортированный список книг из базы данных без признака Арихвный. В переменну d попадает количество записей в списке переменной M. 
#Затем вычисляютсz индекс vn для получения последних 15 записей
#В переменной J строиться список последних 15 записей путем среза из основного списка.
#Затем в цикле запускается вывод 15 записей(или меньше). В этом же цикле присутвует проверка на многотмности. В конце присутвует вывод общего количества записей.
#В данном модуле вызывается функции сортировки(обычная сортировка и сортировка по архивным и не архивным записям),описание классов, функция new_book для заполнения атрибутов класса. 
